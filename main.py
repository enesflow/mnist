...
favicon = pygame.image.load("neural.png")
pygame.display.set_icon(favicon)

my_model = models.load_model(
    "C:\\Users\\enesk\\Desktop\\mnist\\model.h5", compile=False)
...
for i in range(0, 10):
    res.append(0.0)
    cr = res[i]
    texts.append(font.render(f"{i} => ?", True,
                 (255-(cr*255), cr*255, 100), (0, 0, 0)))
...
pygame.display.set_caption("Digit recognition program")
...
def predict():
    ...
    def predict_number_function():
        ...
        for i in range(0, 10):
            cr = res[i]
            texts[i] = font.render(
                f"{i} => {round(cr * 100)}%", True, (255-(cr*255), cr*255, 100), (0, 0, 0))
    predict_thread = threading.Thread(
        target=predict_number_function, name="Predict")
